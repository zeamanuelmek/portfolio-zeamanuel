...
- **ImageSlider molecule** (`components/molecules/ImageSlider.tsx`): Horizontal carousel for case study screenshots. 1 image at a time with crossfade, arrow nav, dot indicators, counter badge, keyboard (arrow keys) and touch swipe support. Uses `Image` atom + `Text` atom. Gold accent on active dot/arrow hover. Single image hides all nav controls. Accepts `aspectRatio` prop (e.g. `"9/16"`, `"16/10"`, `"4/3"`) — **prefer this over `height`** for responsive images: the container scales proportionally to its width so the crop is identical on all screen sizes. `height` still works as a fixed-pixel fallback. CaseStudyPage uses `aspectRatio="9/16"` for portrait mobile app screenshots.
...
**Template Version:** 5.6.0 (ImageSlider responsive fix: aspectRatio prop replaces fixed height for consistent mobile/desktop crop)
